...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 准备工作，记录下容器的启动时间、标记“已启动”状态、初始化占位符资源
			prepareRefresh();

			// Tell the subclass to refresh the internal bean factory.
			//(xml的方式获取)ClassPathXmlApplicationContext
			// 初始化BeanFactory，然后配置文件就会解析成一个个 Bean 定义，注册到 BeanFactory 中，
			// 当然，这里说的 Bean 还没有初始化，只是配置信息都提取出来了，
			// 注册也只是将这些信息都保存到了注册中心(说到底核心是一个 beanName-> beanDefinition 的 map)
			//(注解方式获取)AnnotationConfigApplicationContext
			//只是从原来this()->初始化父类构造器生成的BeanFactory取出来
			ConfigurableListableBeanFactory beanFactory = obtainFreshBeanFactory();

			// Prepare the bean factory for use in this context.
			// 为BeanFactory 设置类加载器，添加几个 BeanPostProcessor，手动注册几个特殊的 bean
			...
			try {
				...
				//如果已经存在则先从当前 BeanFactory 中取
				//否则新建一个默认的SimpleApplicationEventMulticaster作为 applicationEventMulticaster，并且会注册到 BeanFactory 中。
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		...
		// 默认情况下，this.beanFactoryPostProcessors 是返回空的，可以通过customizeContext 方法（该方法是 Spring 提供给开发者的一个扩展点）
		// 添加自定义 BeanFactoryPostProcessor 到 this.beanFactoryPostProcessors 变量中
	...
	protected void registerListeners() {
		...
		for (ApplicationListener<?> listener : getApplicationListeners()) {
			//拿到之前初始化事件广播器--将监听器添加到applicationListeners
		...
		if (earlyEventsToProcess != null) {
			for (ApplicationEvent earlyEvent : earlyEventsToProcess) {
				//调用对应事件监听器的onApplicationEvent方法
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 2.如果beanFactory之前没有注册嵌入值解析器，则注册默认的嵌入值解析器：主要用于注解属性值的解析--占位符。
		if (!beanFactory.hasEmbeddedValueResolver()) {
			beanFactory.addEmbeddedValueResolver(strVal -> getEnvironment().resolvePlaceholders(strVal));
		}

		// Initialize LoadTimeWeaverAware beans early to allow for registering their transformers early.
		// 3.初始化LoadTimeWeaverAware Bean实例对象--aop
		...
		// 会存在两个ClassLoader，一个BeanClassLoader(主),一个临时的(aop时临时的创建ClassLoader帮助我们创建一些中间的ClassLoader)
	...
	protected void finishRefresh() {
		...
		//为此上下文初始化生命周期处理器
		//优先使用用户自定义的生命周期处理器；如果用户没有自定义，则使用默认的 DefaultLifecycleProcessor
		//并注册到beanFactory
		initLifecycleProcessor();

		// Propagate refresh to lifecycle processor first.
		// 首先将刷新完毕事件传播到生命周期处理器（触发isAutoStartup方法为true条件下的SmartLifecycle的start方法）
		getLifecycleProcessor().onRefresh();

		// Publish the final event.
		//最终推送刷新完毕事件到相应监听器即调用了所有实现了ApplicationListener接口的onApplicationEvent方法
		publishEvent(new ContextRefreshedEvent(this));

		// Participate in LiveBeansView MBean, if active.
		//向JMX(Java Management Extensions，即Java管理扩展)里的MBeanServer托管LiveBeans---可以用Jconsole命令打开看到
