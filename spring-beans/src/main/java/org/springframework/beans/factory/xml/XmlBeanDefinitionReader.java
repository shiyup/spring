...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	protected Document doLoadDocument(InputSource inputSource, Resource resource) throws Exception {
		...
		// 2.documentLoader.loadDocument: 加载XML文件，并得到对应的 Documen
	...
	protected int detectValidationMode(Resource resource) {
		...
		try {// 校验resource是否可以打开InputStream
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		// 1.使用DefaultBeanDefinitionDocumentReader 实例化BeanDefinitionDocumentReader
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();
		// 2.记录统计BeanDefinition前的加载个数
		int countBefore = getRegistry().getBeanDefinitionCount();
		// 3.createReaderContext：根据resource创建一个XmlReaderContext（会创建默认的 DefaultNamespaceHandlerResolver）
