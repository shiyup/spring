...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	protected void doRegisterBeanDefinitions(Element root) {
		...
		if (this.delegate.isDefaultNamespace(root)) {
			// 2.处理profile属性（profile 属性主要用于多环境开发）
			// （我们可以在配置文件中同时写上多套配置来适用于开发环境、测试环境、生产环境，
			//  这样可以方便的进行切换开发、部署环境，最常用的就是更换不同的数据库。
			//  具体使用哪个环境在 web.xml 中通过参数 spring.profiles.active 来配置）
	...
	protected void parseBeanDefinitions(Element root, BeanDefinitionParserDelegate delegate) {
		// 1.默认命名空间的处理（默认的命名空间为：http://www.springframework.org/schema/beans，其他都是自定义命名空间）
